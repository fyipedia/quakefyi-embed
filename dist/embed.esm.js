/* quakefyi-embed v1.0.0 | MIT | https://widget.quakefyi.com */
...
async function initGuide(el, config) {
  var _a, _b, _c, _d;
  const shadow = createShadow(el, config);
  const root = createWidgetRoot(shadow, el, "geofyi-guide");
  renderLoading(root);
  const slug = el.dataset.slug || "";
  if (!slug) {
    renderError(root, "Missing data-slug attribute", config);
  ...
  try {
    const data = await fetchAPI(config.apiBase, `guides/${slug}/`);
    const title = String((_b = (_a = data.title) != null ? _a : data.name) != null ? _b : "");
    const excerpt = String((_d = (_c = data.excerpt) != null ? _c : data.description) != null ? _d : "");
    const url = `https://${config.domain}/guides/${esc(slug)}/`;
    root.innerHTML = `
      <div class="geofyi-guide-card">
        <div class="geofyi-guide-title">${esc(title)}</div>
        <div class="geofyi-guide-excerpt">${esc(excerpt)}</div>
      </div>
      <div class="geofyi-view-link"><a href="${url}" target="_blank" rel="noopener">Read on ${esc(config.name)} ${externalLinkIcon}</a></div>
      ${poweredByHTML(config)}
    `;
  } catch (e) {
    renderError(root, "Could not load guide", config);
  }
}

// src/tools/magnitude-viz.ts
function renderMagnitudeViz(container, config) {
  const scales = [1, 2, 3, 4, 5, 6, 7, 8].map((m) => {
    const info = classifyMagnitude(m);
    return `<div class="geofyi-mag-row">
      <span class="geofyi-mag-dot" style="background:${info.color};width:${8 + m * 4}px;height:${8 + m * 4}px"></span>
      <span class="geofyi-mag-label">${m}.0</span>
      <span class="geofyi-badge" style="background:${info.bgColor};color:${info.textColor}">${info.label}</span>
      <span class="geofyi-mag-desc">${info.description}</span>
    </div>`;
  });
  container.innerHTML = `
    <div class="geofyi-tool-header">Richter Magnitude Scale</div>
    <div class="geofyi-tool-body geofyi-mag-scale">
      ${scales.join("")}
    </div>
    ${poweredByHTML(config)}
  `;
...
// src/inline/magnitude-badge.ts
function renderMagnitudeBadge(data, _config) {
  var _a, _b;
  const mag = Number((_b = (_a = data.magnitude) != null ? _a : data.value) != null ? _b : 0);
  const info = classifyMagnitude(mag);
  return `<span class="geofyi-badge" style="background:${info.bgColor};color:${info.textColor}">M${mag.toFixed(1)} ${info.label}</span>`;
}

// src/inline/tsunami-badge.ts
function renderTsunamiBadge(data, _config) {
  var _a;
  const flag = Boolean((_a = data.tsunami_flag) != null ? _a : data.value);
  if (flag) {
    return `<span class="geofyi-badge" style="background:#991b1b;color:#fff">\u26A0\uFE0F Tsunami Warning</span>`;
  }
  return `<span class="geofyi-badge">No Tsunami Warning</span>`;
}

// src/_entry_quakefyi.ts
async function processElement(el, config) {
  if (el.shadowRoot) return;
  const type = el.getAttribute(config.attribute) || "";
  switch (type) {
    case "entity":
    case "earthquake-card":
    ...
    case "search":
      ...
      break;
    case "guide":
      await initGuide(el, config);
      break;
    case "magnitude-viz": {
      const shadow = createShadow(el, config);
      const root = createWidgetRoot(shadow, el, "geofyi-tool");
      renderMagnitudeViz(root, config);
      break;
    }
    case "magnitude-badge": {
      const shadow = createShadow(el, config);
      ...
      root.innerHTML = renderMagnitudeBadge(data, config);
      break;
    }
    case "tsunami-badge": {
      const shadow = createShadow(el, config);
      ...
      root.innerHTML = renderTsunamiBadge(data, config);
      break;
    }
    default:
      console.warn(`[quakefyi-embed] Unknown widget type: ${type}`);
  }
}
...
(function bootstrap() {
  const config = '{"site":"quakefyi","name":"QuakeFYI","domain":"quakefyi.com","accent":"#DC2626","attribute":"data-quakefyi","apiBase":"https://quakefyi.com/api/v1/","searchPath":"/search/","entityName":"Earthquakes","entitySlug":"earthquakes"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"quakefyi","name":"QuakeFYI","domain":"quakefyi.com","accent":"#DC2626","attribute":"data-quakefyi","apiBase":"https://quakefyi.com/api/v1/","searchPath":"/search/","entityName":"Earthquakes","entitySlug":"earthquakes"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"quakefyi","name":"QuakeFYI","domain":"quakefyi.com","accent":"#DC2626","attribute":"data-quakefyi","apiBase":"https://quakefyi.com/api/v1/","searchPath":"/search/","entityName":"Earthquakes","entitySlug":"earthquakes"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"quakefyi","name":"QuakeFYI","domain":"quakefyi.com","accent":"#DC2626","attribute":"data-quakefyi","apiBase":"https://quakefyi.com/api/v1/","searchPath":"/search/","entityName":"Earthquakes","entitySlug":"earthquakes"}'.site;
  const defs = [
    ...
    [`${site}-magnitude-viz`, renderMagnitudeViz, ["magnitude"]]
  ];
